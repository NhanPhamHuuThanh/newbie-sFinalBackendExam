...
@Repository
public interface KeyCapSetRepository extends JpaRepository<KeyCapSet, Integer> {
    ...
    @Query("select k from KeyCapSet k, Manufacturer m where m.id = k.manufacturer.id and m.name = :name")
    List<KeyCapSet> getAllKeyCapSetFromAManufacturer(@Param("name") String name);
